...
function animationOnOff(name) {
    ...
    if (name == "floatingText") document.getElementById("floatingTextAnimBtn").textContent = "浮动文本 " + ((player.options.animations.floatingText) ? "开" : "关")
    else if (name == "bigCrunch") document.getElementById("bigCrunchAnimBtn").textContent = "坍缩 " + ((player.options.animations.bigCrunch) ? "开" : "关")
    else if (name == "tachyonParticles") document.getElementById("tachyonParticleAnimBtn").textContent = "超光速粒子 " + ((player.options.animations.tachyonParticles) ? "开" : "关")
    ...
    if (shiftDown && document.getElementById("eternitystore").style.display !== "none" && document.getElementById("timestudies").style.display !== "none") {
        for (i=0; i<all.length; i++) {
            ...
            if (document.getElementById(all[i]).className.split(" ")[1] !== undefined || all[i] > 220) {
                ...
                var name;
                console.log(all[i])
                if (all[i] == 222 || all[i] == 223 || all[i] == 226 || all[i] == 227 || all[i] == 232 || all[i] == 233) name = "暗"
                else if (all[i] == 221 || all[i] == 224 || all[i] == 225 || all[i] == 228 || all[i] == 231 || all[i] == 234) name = "光"
                else if (tempName.includes("normaldimstudy")) name = "普通维度"
                else if (tempName.includes("infdimstudy")) name = "无限维度"
                else if (tempName.includes("timedimstudy")) name = "时间维度"
                else if (tempName.includes("activestudy")) name = "活跃"
                else if (tempName.includes("passivestudy")) name = "被动"
                else if (tempName.includes("idlestudy")) name = "放置"
