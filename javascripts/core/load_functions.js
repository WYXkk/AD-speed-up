...
if (player.infinitied > 0 && !player.challenges.includes("challenge1")) player.challenges.push("challenge1")
  ...
  sliderText.textContent = "刷新频率: " + player.options.updateRate + "ms";
  ...
  document.getElementById("totaltickgained").textContent = "你获得了 "+shortenDimensions(player.totalTickGained)+" 个时间速率升级."
...
if (player.version < 5) {
  ...
  document.getElementById("floatingTextAnimBtn").textContent = "浮动文本 " + ((player.options.animations.floatingText) ? "开" : "关")
  document.getElementById("bigCrunchAnimBtn").textContent = "坍缩 " + ((player.options.animations.bigCrunch) ? "开" : "关")
  document.getElementById("tachyonParticleAnimBtn").textContent = "超光速粒子 " + ((player.options.animations.tachyonParticles) ? "开" : "关")
  ...
  for(i=1;i<=13;i++) for(j=1;j<=8;j++) if(!player.achievements.includes('r'+i+j)) giveAchievement(allAchievements['r'+i+j])
  for(i=1;i<=3;i++) for(j=1;j<=8;j++) if(!player.achievements.includes('s'+i+j)) giveAchievement(allAchievements['s'+i+j])
}

function load_cloud_save(saveId, cloudPlayer) {
  saves[saveId] = cloudPlayer;

  if (window.location.href.split("//")[1].length > 20) set_save('adFastSaveCnTest', saveId, cloudPlayer);
  else set_save('adFastSaveCn', saveId, cloudPlayer);
...
function load_game(root) {
  if (!root) {
    if (window.location.href.split("//")[1].length > 20) var root = get_save('adFastSaveCnTest');
    else var root = get_save('adFastSaveCn');
  }
...
function save_game(changed, silent) {
  if (window.location.href.split("//")[1].length > 20) set_save('adFastSaveCnTest', currentSave, player);
  else set_save('adFastSaveCn', currentSave, player);
  if (!silent) $.notify(changed ? "游戏加载完成" : "游戏已保存", "info")
}
...
function loadAutoBuyerSettings() {
  for (var i=0; i<9; i++) {
      ...
      if (i == 8 && player.autobuyers[i].target == 10) document.getElementById("toggleBtnTickSpeed").textContent = "买到最大"
      else if (i == 8 && player.autobuyers[i].target !== 10) document.getElementById("toggleBtnTickSpeed").textContent = "单个购买"
      else if (player.autobuyers[i].target > 10) document.getElementById("toggleBtn" + (i+1)).textContent = "买到 10 个"
      else document.getElementById("toggleBtn" + (i+1)).textContent = "买一个"
