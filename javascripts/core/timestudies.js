...
function updateTheoremButtons() {
  ...
  document.getElementById("theoremep").innerHTML = "购买时间定理<br>花费:"+shortenDimensions(player.timestudy.epcost)+" 永恒点数"
  document.getElementById("theoremip").innerHTML = "购买时间定理<br>花费:"+shortenCosts(player.timestudy.ipcost)+" 无限点数"
  document.getElementById("theoremam").innerHTML = "购买时间定理<br>花费:"+shortenCosts(player.timestudy.amcost)
...
function buyTimeStudy(name, cost, check) {
  if (shiftDown && check === undefined) studiesUntil(name);
      ...
      if (name == 131) {
        if (player.replicanti.galaxybuyer) document.getElementById("replicantiresettoggle").textContent = "自动复制器星系 开 (禁用)"
        else document.getElementById("replicantiresettoggle").textContent = "自动复制器星系 关 (禁用)"
      }
      updateTheoremButtons()
      updateTimeStudyButtons()
      drawStudyTree()
...
function respecTimeStudies() {
  ...
  updateTheoremButtons()
  drawStudyTree()
  if (player.replicanti.galaxybuyer) document.getElementById("replicantiresettoggle").textContent = "自动复制器星系 开"
...
finalCheck=()=>{
    if(player.eternityPoints.gte(new Decimal('1e4400'))){
        ...
        alert('3. 游戏中有多少个挑战？\n同一个永恒挑战的不同难度算作同一个挑战.')
        alert('4. 你最多能在购买时间研究上同时花费多少时间定理？\n用于解锁永恒挑战、时间膨胀或者时间维度 5-8 的时间定理不算在内')
        alert('5. 解锁时间膨胀需要的总计时间定理是多少？')
        alert('把 5 个结果加起来, 结果应该一个是在 1 到 100000 之内的整数. 这就是最终答案.')
    }
}