...
function updateInfinityDimensions() {
  if (document.getElementById("infinitydimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
    for (let tier = 1; tier <= 8; ++tier) {
        document.getElementById("infD"+tier).textContent = DISPLAY_NAMES[tier] + "无限维度 x" + shortenMoney(DimensionPower(tier));
...
function switchAutoInf(tier) {
  if (player.infDimBuyers[tier-1]) {
      ...
      document.getElementById("infauto"+tier).textContent = "自动: 关"
  } else {
      player.infDimBuyers[tier-1] = true
      document.getElementById("infauto"+tier).textContent = "自动: 开"
  }
}
...
function toggleAllInfDims() {
  if (player.infDimBuyers[0]) {
      for (var i=1; i<9; i++) {
          ...
          document.getElementById("infauto"+i).textContent = "自动: 关"
      }
  } else {
      for (var i=1; i<9; i++) {
          if (player.eternities - 10>=i) {
              player.infDimBuyers[i-1] = true
              document.getElementById("infauto"+i).textContent = "自动: 开"
          }
      }
  }
}
...
function loadInfAutoBuyers() {
  for (var i=1; i<9; i++) {
      if (player.infDimBuyers[i-1]) document.getElementById("infauto"+i).textContent = "自动: 开"
      else document.getElementById("infauto"+i).textContent = "自动: 关"
  }
}
