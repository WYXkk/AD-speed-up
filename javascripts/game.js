...
function setTheme(name) {
    ...
    if(name === undefined) {
        ...
        document.getElementById("theme").textContent="当前主题：" + name.replace('Metro','现代').replace('Dark','深色').replace('Inverted','反转').replace(' ','，');
    }
...
function updateMoney() {
    ...
    if (player.currentChallenge == "challenge12" || player.currentChallenge == "postc1") element2.textContent = "你有 " + formatValue(player.options.notation, player.matter, 2, 1) + " 物质。"
    if (player.currentChallenge == "postc6") element2.textContent = "你有 " + formatValue(player.options.notation, Decimal.pow(player.matter,20), 2, 1) + " 物质。"; //TODO
}

function updateCoinPerSec() {
    var element = document.getElementById("coinsPerSec");
    if (player.currentChallenge == "challenge3" || player.currentChallenge == "postc1") {
      element.textContent = '你每秒获得 ' + shortenDimensions(getDimensionProductionPerSecond(1).times(player.chall3Pow)) + ' 反物质。'
    } else if (player.currentChallenge == "challenge7") {
      element.textContent = '你每秒获得 ' + (shortenDimensions(getDimensionProductionPerSecond(1).plus(getDimensionProductionPerSecond(2)))) + ' 反物质。'
    } else {
      element.textContent = '你每秒获得 ' + shortenDimensions(getDimensionProductionPerSecond(1)) + ' 反物质。'
    }
}
...
function updateDimensions() {
    if (document.getElementById("antimatterdimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
        ...
        for (let tier = 1; tier <= 8; ++tier) {
            ...
            document.getElementById(name + "D").childNodes[0].nodeValue = DISPLAY_NAMES[tier] + "维度 x" + formatValue(player.options.notation, getDimensionFinalMultiplier(tier), 1, 1);
        ...
        if (player.currentChallenge == "challenge4" ? shiftRequirement.tier < 6 : shiftRequirement.tier < 8) {
            document.getElementById("resetLabel").textContent = '维度提升 ('+ player.resets +'): 需要 ' + shiftRequirement.amount + " " + DISPLAY_NAMES[shiftRequirement.tier] + "维度"
        }
        ...
        if (player.currentChallenge == "challenge4" ? player.resets > 2 : player.resets > 3) {
            document.getElementById("softReset").textContent = "重置并获得对维度的增益"
        } else {
            document.getElementById("softReset").textContent = "重置来解锁新维度"
        }
        ...
        var galString = ""
        if (player.galaxies >= 800) galString += "极远反物质星系 (";
        else if (player.galaxies >= getGalaxyCostScalingStart() || player.currentEternityChall === "eterc5") galString += "遥远反物质星系 (";
    ...
    if (canBuyTickSpeed() || player.currentEternityChall == "eterc9") {
        ...
        if (tickmult < 1e-9) document.getElementById("tickLabel").textContent = "将计时间隔除以 " + shortenDimensions(1 / tickmult) + '.'
        else {
            var places = 0
            if (tickmult < 0.2) places = Math.floor(Math.log10(Math.round(1/tickmult)))
            document.getElementById("tickLabel").textContent = '将计时间隔减少 ' + ((1 - tickmult) * 100).toFixed(places) + '%.'
        }
    ...
    if (player.bestInfinityTime == 9999999999) {
        ...
        document.getElementById("bestInfinity").textContent = "你最快的无限用时 " + timeDisplay(player.bestInfinityTime) + "."
        document.getElementById("thisInfinity").textContent = "你在本次无限中花了 " + timeDisplay(player.thisInfinityTime) + "."
        if (player.infinityPoints.equals(1)) {
            document.getElementById("infinityPoints1").textContent = "你有 1 无限点数."
            document.getElementById("infinityPoints2").textContent = "你有 1 无限点数."
        }
        else {
            document.getElementById("infinityPoints1").innerHTML = "你有 <span class=\"IPAmount1\">"+shortenDimensions(player.infinityPoints)+"</span> 无限点数."
            document.getElementById("infinityPoints2").innerHTML = "你有 <span class=\"IPAmount2\">"+shortenDimensions(player.infinityPoints)+"</span> 无限点数."
        }
        if (player.infinitied == 1) document.getElementById("infinitied").textContent = "你无限了 1 次."
        else document.getElementById("infinitied").textContent = "你无限了 " + player.infinitied.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " 次."
        if (player.infinitiedBank > 0) document.getElementById("infinitied").textContent = "你在这次永恒中无限了 " + player.infinitied.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " 次."

    }

    if (document.getElementById("stats").style.display == "block" && document.getElementById("statistics").style.display == "block") {
        document.getElementById("totalmoney").textContent = '你一共制造了 ' + shortenMoney(player.totalmoney) + ' 反物质.'
        document.getElementById("totalresets").textContent = '你进行了 ' + player.resets + ' 次维度提升.'
        document.getElementById("galaxies").textContent = '你有 ' + Math.round(player.galaxies) + ' 个反物质星系.'
        document.getElementById("totalTime").textContent = "你已经玩了 " + timeDisplay(player.totalTimePlayed) + "."
        ...
        if (player.eternities == 0) {
            ...
            document.getElementById("eternitied").textContent = "你永恒了 " + player.eternities.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " 次."
            document.getElementById("besteternity").textContent = "你在本次永恒中花了 "+timeDisplay(player.thisEternity)+"."
            document.getElementById("thiseternity").textContent = "你最快的永恒用时 "+timeDisplay(player.bestEternity)+"."
        }
    }

    if (document.getElementById("infinity").style.display == "block") {
        if (document.getElementById("preinf").style.display == "block") {
            document.getElementById("infi11").innerHTML = "普通维度根据游玩时长获得加成<br>当前: " + (Math.pow(0.5 * player.totalTimePlayed * devSpeed / 600, 0.15)).toFixed(2) + "x<br>花费: 1 无限点数"
            document.getElementById("infi12").innerHTML = "第一和第八维度根据无限次数获得加成<br>当前: " + formatValue(player.options.notation, dimMults(), 1, 1) + "x<br>花费: 1 无限点数"
            document.getElementById("infi13").innerHTML = "第三和第六维度根据无限次数获得加成<br>当前: " + formatValue(player.options.notation, dimMults(), 1, 1) + "x<br>花费: 1 无限点数"
            document.getElementById("infi22").innerHTML = "第二和第七维度根据无限次数获得加成<br>当前: " + formatValue(player.options.notation, dimMults(), 1, 1) + "x<br>花费: 1 无限点数"
            document.getElementById("infi23").innerHTML = "第四和第五维度根据无限次数获得加成<br>当前: " + formatValue(player.options.notation, dimMults(), 1, 1) + "x<br>花费: 1 无限点数"
            document.getElementById("infi31").innerHTML = "普通维度根据在本次无限中的时间获得加成<br>当前: " + Decimal.max(Math.pow(player.thisInfinityTime * devSpeed / 2400, 0.25), 1).toFixed(2) + "x<br>花费: 3 无限点数"
            document.getElementById("infi32").innerHTML = "根据未花费的无限点数给第一维度加成<br>当前: " + formatValue(player.options.notation, player.infinityPoints.dividedBy(2).pow(1.5).plus(1), 2, 2) + "x<br>花费: 5 无限点数"
            document.getElementById("infi34").innerHTML = "根据最快的无限生成无限点数 <br>当前: "+shortenDimensions(player.infMult.times(kongIPMult))+" 每 " + timeDisplay(player.bestInfinityTime*10) + "<br>花费: 10 无限点数"
        }
        else if (document.getElementById("postinf").style.display == "block") {
            document.getElementById("postinfi11").innerHTML = "根据总计生产的反物质加成所有维度<br>当前: "+ Math.pow(player.totalmoney.e+1, 0.5).toFixed(2)+"x<br>花费: "+shortenCosts(1e4)+" 无限点数"
            document.getElementById("postinfi21").innerHTML = "根据当前反物质加成所有维度<br>当前: "+ Math.pow(player.money.e+1, 0.5).toFixed(2)+"x<br>花费: "+shortenCosts(5e4)+" 无限点数"
            document.getElementById("postinfi31").innerHTML = "时间速率升级价格上涨 <br>"+player.tickSpeedMultDecrease+"x -> "+(player.tickSpeedMultDecrease-1)+"x<br>花费: "+shortenDimensions(player.tickSpeedMultDecreaseCost) +" 无限点数"
            if (player.tickSpeedMultDecrease <= 2) document.getElementById("postinfi31").innerHTML = "时间速率升级价格上涨 <br>"+player.tickSpeedMultDecrease+"x"
            document.getElementById("postinfi22").innerHTML = "根据完成的成就数加成所有维度 <br>当前: "+achievementMult.toFixed(2)+"x<br>花费: "+shortenCosts(1e6)+" 无限点数"
            document.getElementById("postinfi12").innerHTML = "根据无限次数加成所有维度 <br>当前: "+(1+Math.log10(getInfinitied()+1)*10).toFixed(2)+"x<br>花费: "+shortenCosts(1e5)+" 无限点数"
            if (player.timestudy.studies.includes(31)) document.getElementById("postinfi12").innerHTML = "根据无限次数加成所有维度 <br>当前: "+shortenMoney(Math.pow((Math.log10(getInfinitied()+1)*10).toFixed(2), 4))+"x<br>花费: "+shortenCosts(1e5)+" 无限点数"
            document.getElementById("postinfi41").innerHTML = "星系效果增强 50% <br>花费: "+shortenCosts(5e11)+" 无限点数"
            document.getElementById("postinfi32").innerHTML = "根据最慢的普通挑战完成时间加成所有维度<br>当前:"+Decimal.max(10*3000/worstChallengeTime, 1).toFixed(2)+"x<br>花费: "+shortenCosts(1e7)+" 无限点数"
            document.getElementById("postinfi42").innerHTML = "维度价格上涨 <br>"+player.dimensionMultDecrease+"x -> "+(player.dimensionMultDecrease-1)+"x<br>花费: "+shortenCosts(player.dimensionMultDecreaseCost) +" 无限点数"
        ...
        infGain *= Math.floor(Math.min(1000,Math.max(5,Math.pow(player.infinitied,2/3))));
            document.getElementById("postinfi13").innerHTML = "根据最快的无限生成无限次数 <br>"+ infGain +" 次无限每 "+timeDisplay(player.bestInfinityTime*5)+ " <br>花费: "+shortenCosts(20e6)+" 无限点数"
            document.getElementById("postinfi23").innerHTML = "解锁批量购买维度提升 <br>花费: "+shortenCosts(5e9)+" 无限点数"
            document.getElementById("postinfi33").innerHTML = "所有自动购买器速度翻倍 <br>花费:"+shortenCosts(1e15)+" 无限点数"
            if (player.dimensionMultDecrease <= 3) document.getElementById("postinfi42").innerHTML = "维度价格上涨 <br>"+player.dimensionMultDecrease.toFixed(1)+"x"

            document.getElementById("offlineProd").innerHTML = "每分钟获得你无限时的无限点数的 "+player.offlineProd+"% > "+Math.max(Math.max(5, player.offlineProd + 5), Math.min(50, player.offlineProd + 5))+"%, 离线时也生效<br>当前: "+shortenMoney(gainedInfinityPoints().times(player.offlineProd/100)) +"无限点数每分钟<br> 花费: "+shortenCosts(player.offlineProdCost)+" 无限点数"
            if (player.offlineProd == 50) document.getElementById("offlineProd").innerHTML = "每分钟获得你无限时的无限点数的 "+player.offlineProd+"%, 离线时也生效<br>当前: "+shortenMoney(gainedInfinityPoints().times(player.offlineProd/100)) +" 无限点数每分钟"
        }
    }
    ...
    if (document.getElementById("eternityupgrades").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        document.getElementById("eter1").innerHTML = "根据未花费的永恒点数加成无限维度 (x+1)<br>当前: "+shortenMoney(player.eternityPoints.plus(1))+"x<br>花费: 5 永恒点数"
        document.getElementById("eter2").innerHTML = "根据永恒次数加成无限维度 ((x/200)^log4(2x))<br>当前: "+shortenMoney(Decimal.pow(Math.min(player.eternities, 100000)/200 + 1, Math.log(Math.min(player.eternities, 100000)*2+1)/Math.log(4)).times(new Decimal((player.eternities-100000)/200 + 1).times(Math.log((player.eternities- 100000)*2+1)/Math.log(4)).max(1)))+"x<br>花费: 10 永恒点数"
        document.getElementById("eter3").innerHTML = "根据无限挑战时长总和加成无限维度<br>当前: "+shortenMoney(Decimal.pow(2,300/Math.max(infchallengeTimes, player.achievements.includes("r112") ? 6.1 : 7.5)))+"x<br>花费: "+shortenCosts(50e3)+" 永恒点数"
        document.getElementById("eter4").innerHTML = "你的成就效果也影响时间维度"+"<br>花费: "+shortenCosts(1e16)+" 永恒点数"
        document.getElementById("eter5").innerHTML = "时间维度效果乘以未花费的时间定理"+"<br>花费: "+shortenCosts(1e40)+" 永恒点数"
        document.getElementById("eter6").innerHTML = "时间维度效果乘以游玩的天数"+"<br>花费: "+shortenCosts(1e50)+" 永恒点数"
        document.getElementById("eterex").innerHTML = "每秒获得永恒时的永恒点的 1%"+"<br>花费: "+shortenCosts(1000)+" 永恒点数"
        document.getElementById("eterex2").innerHTML = "自动购买5倍永恒点数的升级"+"<br>花费: "+shortenCosts(1e6)+" 永恒点数"
        document.getElementById("eterex3").innerHTML = "自动购买时间定理和时间维度"+"<br>花费: "+shortenCosts(1e9)+" 永恒点数"
    }
    ...
    if (document.getElementById("dilation").style.display == "block") {
        if (player.dilation.active) {
            if (Math.pow(Decimal.log10(player.money) / 400, 1.5) * getTachyonGainMultiplier() - player.dilation.totalTachyonParticles <= 0) {
                document.getElementById("enabledilation").innerHTML = "退出时间膨胀<br>达到 " + shortenMoney(Decimal.pow(10, Math.pow(player.dilation.tachyonParticles * Math.pow(400, 1.5) / getTachyonGainMultiplier(), 2/3))) + " 反物质来获得更多超光速粒子"
            } else {
                document.getElementById("enabledilation").textContent = "退出时间膨胀"
            }
        }
        else document.getElementById("enabledilation").textContent = "进入时间膨胀"
    }
}

function updateCosts() {
    document.getElementById("first").textContent = '花费: ' + shortenCosts(player.firstCost);
    document.getElementById("second").textContent = '花费: ' + shortenCosts(player.secondCost);
    document.getElementById("third").textContent = '花费: ' + shortenCosts(player.thirdCost);
    document.getElementById("fourth").textContent = '花费: ' + shortenCosts(player.fourthCost);
    document.getElementById("fifth").textContent = '花费: ' + shortenCosts(player.fifthCost);
    document.getElementById("sixth").textContent = '花费: ' + shortenCosts(player.sixthCost);
    document.getElementById("seventh").textContent = '花费: ' + shortenCosts(player.seventhCost);
    document.getElementById("eight").textContent = '花费: ' + shortenCosts(player.eightCost);

    document.getElementById("firstMax").textContent = '买到 10 个, 花费: ' + shortenCosts(player.firstCost.times(10 - dimBought(1)));
    document.getElementById("secondMax").textContent = '买到 10 个, 花费: ' + shortenCosts(player.secondCost.times(10 - dimBought(2)));
    document.getElementById("thirdMax").textContent = '买到 10 个, 花费: ' + shortenCosts(player.thirdCost.times(10 - dimBought(3)));
    document.getElementById("fourthMax").textContent = '买到 10 个, 花费: ' + shortenCosts(player.fourthCost.times(10 - dimBought(4)));
    document.getElementById("fifthMax").textContent = '买到 10 个, 花费: ' + shortenCosts(player.fifthCost.times(10 - dimBought(5)));
    document.getElementById("sixthMax").textContent = '买到 10 个, 花费: ' + shortenCosts(player.sixthCost.times(10 - dimBought(6)));
    document.getElementById("seventhMax").textContent = '买到 10 个, 花费: ' + shortenCosts(player.seventhCost.times(10 - dimBought(7)));
    document.getElementById("eightMax").textContent = '买到 10 个, 花费: ' + shortenCosts(player.eightCost.times(10 - dimBought(8)));

    document.getElementById("tickSpeed").textContent = '花费: ' + shortenCosts(player.tickSpeedCost);


    for (var i=1; i<=8; i++) {

        document.getElementById("infMax"+i).textContent = "花费: " + shortenCosts(player["infinityDimension"+i].cost) + " 无限点数"
    }

    for (var i=1; i<=8; i++) {

        document.getElementById("timeMax"+i).textContent = "花费: " + shortenDimensions(player["timeDimension"+i].cost) + " 永恒点数"
    }
}
...
function updateChallenges() {
    try {
        ...
        for (var i=0; i < player.challenges.length; i++) {
            ...
            document.getElementById(player.challenges[i]).textContent = "完成"
        }

        if (player.currentChallenge != "") {
            document.getElementById(player.currentChallenge).className = "onchallengebtn"
            document.getElementById(player.currentChallenge).textContent = "正在挑战"
        }
...
function updateEternityChallenges() {
    ...
    for (var property in player.eternityChalls) {
        ...
        if (player.eternityChalls[property] < 5) {
            document.getElementById(property).textContent = "锁定"
            document.getElementById(property).className = "lockedchallengesbtn"
        }
        else {
            document.getElementById(property).textContent = "完成"
    ...
    if (player.eternityChallUnlocked !== 0) {
        ...
        for (i=1; i<13; i++) {
            if (player.eternityChalls["eterc"+i] !== 5) {
                document.getElementById("eterc"+i).textContent = "锁定"
    ...
    if (player.currentEternityChall !== "") {
        document.getElementById(player.currentEternityChall).textContent = "正在挑战"
...
function toggleChallengeRetry() {
    if (player.options.retryChallenge) {
        ...
        document.getElementById("retry").textContent = "自动重试挑战 关"
    } else {
        player.options.retryChallenge = true
        document.getElementById("retry").textContent = "自动重试挑战 开"
    }
}
...
document.getElementById("challengeconfirmation").onclick = function () {
    if (!player.options.challConf) {
        ...
        document.getElementById("challengeconfirmation").textContent = "挑战前确认 关"
    } else {
        player.options.challConf = false;
        document.getElementById("challengeconfirmation").textContent = "挑战前确认 开"
    }
}
...
document.getElementById("infiMult").onclick = function() {
    if (player.infinityUpgrades.includes("skipResetGalaxy") && player.infinityUpgrades.includes("passiveGen") && player.infinityUpgrades.includes("galaxyBoost") && player.infinityUpgrades.includes("resetBoost") && player.infinityPoints.gte(player.infMultCost)) {
        ...
        document.getElementById("infiMult").innerHTML = "获得的无限点数乘 2 <br>当前: "+shorten(player.infMult.times(kongIPMult)) +"x<br>花费: "+shortenCosts(player.infMultCost)+" 无限点数"
...
function updateEternityUpgrades() {
    ...
    document.getElementById("eterex").className = (player.eternityUpgrades.includes(7)) ? "eternityupbtnbought" : (player.eternityPoints.gte(1000)) ? "eternityupbtn" : "eternityupbtnlocked"
    document.getElementById("eterex2").className = (player.eternityUpgrades.includes(8)) ? "eternityupbtnbought" : (player.eternityPoints.gte(1e6)) ? "eternityupbtn" : "eternityupbtnlocked"
    document.getElementById("eterex3").className = (player.eternityUpgrades.includes(9)) ? "eternityupbtnbought" : (player.eternityPoints.gte(1e9)) ? "eternityupbtn" : "eternityupbtnlocked"
}
...
function buyEPMult() {
    if (player.eternityPoints.gte(player.epmultCost)) {
        ...
        document.getElementById("epmult").innerHTML = "获得的永恒点数乘 5<p>当前: "+shortenDimensions(player.epmult)+"x<p>花费: "+shortenDimensions(player.epmultCost)+" 永恒点数"
        updateEternityUpgrades()
    }
}
...
document.getElementById("postinfi31").onclick = function() {
    if (player.infinityPoints.gte(player.tickSpeedMultDecreaseCost) && player.tickSpeedMultDecrease > 2) {
        ...
        document.getElementById("postinfi31").innerHTML = "时间速率升级价格上涨 <br>"+player.tickSpeedMultDecrease+"x -> "+(player.tickSpeedMultDecrease-1)+"x<br>花费: "+shortenCosts(player.tickSpeedMultDecreaseCost) +" 无限点数"
        if (player.tickSpeedMultDecrease <= 2) document.getElementById("postinfi31").innerHTML = "时间速率升级价格上涨 <br>"+player.tickSpeedMultDecrease+"x"
    }
}
...
function updateInfCosts() {
    ...
    if (document.getElementById("replicantis").style.display == "block" && document.getElementById("infinity").style.display == "block") {
        if (player.replicanti.chance < 1) document.getElementById("replicantichance").innerHTML = "复制概率: "+Math.round(player.replicanti.chance*100)+"%<br>+"+1+"% 花费: "+shortenCosts(player.replicanti.chanceCost)+" 无限点数"
        ...
        if (player.timestudy.studies.includes(233)) {
            if (replGalOver !== 0) document.getElementById("replicantimax").innerHTML = "最大复制器星系: "+player.replicanti.gal+"+"+replGalOver+"<br>+1 花费: "+shortenCosts(player.replicanti.galCost.dividedBy(player.replicanti.amount.pow(0.3)))+" 无限点数"
            else document.getElementById("replicantimax").innerHTML = "最大复制器星系: "+player.replicanti.gal+"<br>+1 花费: "+shortenCosts(player.replicanti.galCost.dividedBy(player.replicanti.amount.pow(0.3)))+" 无限点数"
        } else {
            if (replGalOver !== 0) document.getElementById("replicantimax").innerHTML = "最大复制器星系: "+player.replicanti.gal+"+"+replGalOver+"<br>+1 花费: "+shortenCosts(player.replicanti.galCost)+" 无限点数"
            else document.getElementById("replicantimax").innerHTML = "最大复制器星系: "+player.replicanti.gal+"<br>+1 花费: "+shortenCosts(player.replicanti.galCost)+" 无限点数"
        }
        document.getElementById("replicantiunlock").innerHTML = "解锁复制器<br>花费: "+shortenCosts(1e140)+" 无限点数"
        let extraGals = 0
        if (player.timestudy.studies.includes(225)) extraGals += Math.floor(player.replicanti.amount.e / 1000)
        if (player.timestudy.studies.includes(226)) extraGals += Math.floor(player.replicanti.gal / 15)
        if (extraGals !== 0) document.getElementById("replicantireset").innerHTML = "重置复制器数量，但是获得一个复制器星系<br>获得了 "+player.replicanti.galaxies + "+"+extraGals+ " 个复制器星系."
        else document.getElementById("replicantireset").innerHTML = (player.replicanti.galaxies !== 1) ? "重置复制器数量，但是获得一个复制器星系<br>获得了 "+player.replicanti.galaxies + " 个复制器星系." : "重置复制器数量，但是获得一个复制器星系<br>获得了 "+player.replicanti.galaxies + " 个复制器星系."
    ...
    if (document.getElementById("timestudies").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        ...
        document.getElementById("32desc").textContent = "你获得 "+Math.max(player.resets, 1)+" 倍无限次数 (根据维度提升数量)"
        document.getElementById("51desc").textContent = "你获得 "+shortenCosts(1e15)+" 倍无限点数"
        ...
        document.getElementById("91desc").textContent = "当前: "+shortenMoney(Decimal.pow(10, Math.min(player.thisEternity * devSpeed, 18000)/60))+"x"
        ...
        document.getElementById("121desc").textContent = "当前: "+((253 - averageEp.dividedBy(player.epmult).dividedBy(10).dividedBy(devSpeed).min(248).max(3))/5).toFixed(1)+"x"
        document.getElementById("123desc").textContent = "当前: "+Math.sqrt(1.39*player.thisEternity * devSpeed/10).toFixed(1)+"x"
        ...
        document.getElementById("142desc").textContent = "你获得 "+shortenCosts(1e25)+" 倍无限点数"
        document.getElementById("143desc").textContent = "当前: "+shortenMoney(Decimal.pow(15, Math.log(player.thisInfinityTime * devSpeed)*Math.pow(player.thisInfinityTime * devSpeed, 0.125)))+"x"
        document.getElementById("151desc").textContent = "所有时间维度获得 " + shortenCosts(1e4)+"x 加成"
        document.getElementById("161desc").textContent = "所有普通维度获得 " + shortenCosts(new Decimal("1e616"))+"x 加成"
        document.getElementById("162desc").textContent = "所有无穷维度获得 " +shortenCosts(1e11)+"x 加成"
        document.getElementById("192desc").textContent = "你可以获得超过 "+shortenMoney(Number.MAX_VALUE)+" 复制器, 但是复制间隔会根据已有复制器逐渐上升"
        ...
        if (player.etercreq !== 1) document.getElementById("ec1unl").innerHTML = "永恒挑战 1<span>要求: "+(ECTimesCompleted("eterc1")+1)*20000+" 永恒次数<span>花费: 30 时间定理"
        else document.getElementById("ec1unl").innerHTML = "永恒挑战 1<span>花费: 30 时间定理"
        if (player.etercreq !== 2) document.getElementById("ec2unl").innerHTML = "永恒挑战 2<span>要求: 时间维度提供 "+(1300+(ECTimesCompleted("eterc2")*150))+" 个时间速率升级<span>花费: 35 时间定理"
        else document.getElementById("ec2unl").innerHTML = "永恒挑战 2<span>花费: 35 时间定理"
        if (player.etercreq !== 3) document.getElementById("ec3unl").innerHTML = "永恒挑战 3<span>要求: "+(17300+(ECTimesCompleted("eterc3")*1250))+" 个第八维度<span>花费: 40 时间定理"
        else document.getElementById("ec3unl").innerHTML = "永恒挑战 3<span>花费: 40 时间定理"
        if (player.etercreq !== 4) document.getElementById("ec4unl").innerHTML = "永恒挑战 4<span>要求: "+(1e8 + (ECTimesCompleted("eterc4")*5e7)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+" 次无限<span>花费: 70 时间定理"
        else document.getElementById("ec4unl").innerHTML = "永恒挑战 4<span>花费: 70 时间定理"
        if (player.etercreq !== 5) document.getElementById("ec5unl").innerHTML = "永恒挑战 5<span>要求: "+(160+(ECTimesCompleted("eterc5")*14))+" 个星系<span>花费: 130 时间定理"
        else document.getElementById("ec5unl").innerHTML = "永恒挑战 5<span>花费: 130 时间定理"
        if (player.etercreq !== 6) document.getElementById("ec6unl").innerHTML = "永恒挑战 6<span>要求: "+(40+(ECTimesCompleted("eterc6")*5))+" 个复制器星系<span>花费: 85 时间定理"
        else document.getElementById("ec6unl").innerHTML = "永恒挑战 6<span>花费: 85 时间定理"
        if (player.etercreq !== 7) document.getElementById("ec7unl").innerHTML = "永恒挑战 7<span>要求: "+shortenCosts(new Decimal("1e500000").times(new Decimal("1e300000").pow(ECTimesCompleted("eterc7"))))+" 反物质 <span>花费: 115 时间定理"
        else document.getElementById("ec7unl").innerHTML = "永恒挑战 7<span>花费: 115 时间定理"
        if (player.etercreq !== 8) document.getElementById("ec8unl").innerHTML = "永恒挑战 8<span>要求: "+shortenCosts(new Decimal("1e4000").times(new Decimal("1e1000").pow(ECTimesCompleted("eterc8"))))+" 无限点数 <span>花费: 115 时间定理"
        else document.getElementById("ec8unl").innerHTML = "永恒挑战 8<span>花费: 115 时间定理"
        if (player.etercreq !== 9) document.getElementById("ec9unl").innerHTML = "永恒挑战 9<span>要求: "+shortenCosts(new Decimal("1e17500").times(new Decimal("1e2000").pow(ECTimesCompleted("eterc9"))))+" 无限之力<span>花费: 415 时间定理"
        else document.getElementById("ec9unl").innerHTML = "永恒挑战 9<span>花费: 415 时间定理"
        if (player.etercreq !== 10) document.getElementById("ec10unl").innerHTML = "永恒挑战 10<span>要求: "+shortenCosts(new Decimal("1e100").times(new Decimal("1e20").pow(ECTimesCompleted("eterc10"))))+" 永恒点数<span>花费: 550 时间定理"
        else document.getElementById("ec10unl").innerHTML = "永恒挑战 10<span>花费: 550 时间定理"

        document.getElementById("ec11unl").innerHTML = "永恒挑战 11<span>要求: 只使用普通维度路径<span>花费: 1 时间定理"
        document.getElementById("ec12unl").innerHTML = "永恒挑战 12<span>要求: 只使用时间维度路径<span>花费: 1 时间定理"

        if (player.dilation.studies.includes(1)) document.getElementById("dilstudy1").innerHTML = "解锁时间膨胀<span>要求: 完成 5 次永恒挑战 11,12, 总计 13000 时间定理<span>花费: 5000 时间定理"
        else document.getElementById("dilstudy1").innerHTML = "解锁时间膨胀<span>要求: 完成 5 次永恒挑战 11,12, 总计 13000 时间定理<span>花费: 5000 时间定理"
    }
}
...
function upgradeReplicantiChance() {
    if (player.infinityPoints.gte(player.replicanti.chanceCost) && player.replicanti.chance < 1 && player.eterc8repl !== 0) {
        ...
        document.getElementById("eterc8repl").textContent = "你还可以购买 "+player.eterc8repl+" 次."
    }
}
...
function replicantiGalaxyAutoToggle() {
    if (player.replicanti.galaxybuyer) {
        ...
        if (player.timestudy.studies.includes(131)) document.getElementById("replicantiresettoggle").textContent = "自动复制器星系 关 (禁用)"
        else document.getElementById("replicantiresettoggle").textContent = "自动复制器星系 关"
    } else {
        player.replicanti.galaxybuyer = true
        if (player.timestudy.studies.includes(131)) document.getElementById("replicantiresettoggle").textContent = "自动复制器星系 开 (禁用)"
        else document.getElementById("replicantiresettoggle").textContent = "自动复制器星系 开"
    }
}
...
function infMultAutoToggle() {
    if (player.infMultBuyer) {
        ...
        document.getElementById("infmultbuyer").textContent = "自动购买无限点数乘数 关"
    } else {
        player.infMultBuyer = true
        document.getElementById("infmultbuyer").textContent = "自动购买无限点数乘数 开"
    }
}
...
function toggleCrunchMode() {
    if (player.autoCrunchMode == "amount") {
        ...
        document.getElementById("limittext").textContent = "坍缩间隔时间:"
    } else if (player.autoCrunchMode == "time"){
        player.autoCrunchMode = "relative"
        document.getElementById("togglecrunchmode").textContent = "自动坍缩模式: 上一次坍缩的倍数"
        document.getElementById("limittext").textContent = "上一次坍缩的倍数:"
    } else {
        player.autoCrunchMode = "amount"
        document.getElementById("togglecrunchmode").textContent = "自动坍缩模式: 数量"
        document.getElementById("limittext").textContent = "自动坍缩需要的无限点数量:"
    }
}
...
function toggleEternityConf() {
    if (player.options.eternityconfirm) {
        ...
        document.getElementById("eternityconf").textContent = "永恒前确认 关"
    } else {
        player.options.eternityconfirm = true
        document.getElementById("eternityconf").textContent = "永恒前确认 开"
    }
}
...
function toggleReplAuto(i) {
    if (i == "chance") {
        if (player.replicanti.auto[0]) {
            ...
            document.getElementById("replauto1").textContent = "自动购买 关"
        } else {
            player.replicanti.auto[0] = true
            document.getElementById("replauto1").textContent = "自动购买 开"
        }
    } else if (i == "interval") {
        if (player.replicanti.auto[1]) {
            player.replicanti.auto[1] = false
            document.getElementById("replauto2").textContent = "自动购买 关"
        } else {
            player.replicanti.auto[1] = true
            document.getElementById("replauto2").textContent = "自动购买 开"
        }
    } else if (i == "galaxy") {
        if (player.replicanti.auto[2]) {
            player.replicanti.auto[2] = false
            document.getElementById("replauto3").textContent = "自动购买 关"
        } else {
            player.replicanti.auto[2] = true
            document.getElementById("replauto3").textContent = "自动购买 开"
        }
    }
}
...
function toggleCommas() {
    ...
    if (player.options.commas) document.getElementById("commas").textContent = "指数用逗号表示"
    else document.getElementById("commas").textContent = "指数用相同计数法表示"
}
...
toggleAutobuyerTarget = function(id) {
    if (player.autobuyers[id-1].target == id) {
        ...
        document.getElementById("toggleBtn" + id).textContent = "买到 10 个"
    } else {
        player.autobuyers[id-1].target = id
        document.getElementById("toggleBtn" + id).textContent = "买一个"
    }
}
...
document.getElementById("toggleBtnTickSpeed").onclick = function () {
    if (player.autobuyers[8].target == 1) {
        ...
        document.getElementById("toggleBtnTickSpeed").textContent = "买到最大"
    } else {
        player.autobuyers[8].target = 1
        document.getElementById("toggleBtnTickSpeed").textContent = "单个购买"
    }
}
...
document.getElementById("importbtn").onclick = function () {
    var save_data = prompt("输入你的存档 (当前的存档会被覆盖!)");
...
document.getElementById("reset").onclick = function () {
    if (forceHardReset) {
        if (window.location.href.split("//")[1].length > 20) set_save('adFastSaveCnTest', currentSave, defaultStart);
        else set_save('adFastSaveCn', currentSave, defaultStart);
        player = defaultStart
        infDimPow = 1;
        save_game();
        load_game();
...
function breakInfinity() {
    ...
    if (player.break && !player.currentChallenge.includes("post")) {
        player.break = false
        document.getElementById("break").textContent = "打破无限"
    } else {
        player.break = true
...
function gainedInfinityPoints() {
    ...
    if (player.timestudy.studies.includes(143)) ret = ret.times(Decimal.pow(15, Math.log(player.thisInfinityTime*devSpeed+1)*Math.pow(player.thisInfinityTime*devSpeed+1, 0.125)))
    if (player.achievements.includes("r116")) ret = ret.times(Decimal.pow(2, Math.log10(getInfinitied()+1)))
    if (player.achievements.includes("r125")) ret = ret.times(Decimal.pow(2, Math.log(player.thisInfinityTime*devSpeed+1)*Math.pow(player.thisInfinityTime*devSpeed+1, 0.11)))
...
function gainedEternityPoints() {
    ...
    if (player.timestudy.studies.includes(121)) ret = ret.times(((253 - averageEp.dividedBy(player.epmult).dividedBy(10).dividedBy(devSpeed).min(248).max(3))/5)) //x300 if tryhard, ~x60 if not
    else if (player.timestudy.studies.includes(122)) ret = ret.times(35)
    else if (player.timestudy.studies.includes(123)) ret = ret.times(Math.sqrt(1.39*player.thisEternity * devSpeed/10))
...
document.getElementById("sacrifice").onclick = function () {
    if (!document.getElementById("confirmation").checked) {
        if (!confirm("维度献祭会清零你的第一到第七维度，并获得第八维度的加成。一段时间后才能恢复生产。")) {
            return false;
        }
    }
...
function updateAutobuyers() {
    ...
    document.getElementById("toggleBtn" + tier).style.display = "inline-block";
        ...
        else document.getElementById("buyerBtn" + tier).innerHTML = "间隔缩短 40% <br>花费: " + shortenDimensions(player.autobuyers[tier-1].cost) + " 无限点数"
        }
    }
    ...
    document.getElementById("buyerBtnTickSpeed").innerHTML = "间隔缩短 40% <br>花费: " + player.autobuyers[8].cost + " 无限点数"
    document.getElementById("buyerBtnDimBoost").innerHTML = "间隔缩短 40% <br>花费: " + player.autobuyers[9].cost + " 无限点数"
    document.getElementById("buyerBtnGalaxies").innerHTML = "间隔缩短 40% <br>花费: " + player.autobuyers[10].cost + " 无限点数"
    document.getElementById("buyerBtnInf").innerHTML = "间隔缩短 40% <br>花费: " + player.autobuyers[11].cost + " 无限点数"
...
function toggleHotkeys() {
    if (player.options.hotkeys) {
        ...
        document.getElementById("hotkeys").textContent = "启用快捷键"
    } else {
...
document.getElementById("challengetime2").textContent = "普通挑战  " + 2 + " 时间记录: " + timeDisplayShort(player.challengeTimes[0])
    document.getElementById("challengetime3").textContent = "普通挑战  " + 3 + " 时间记录: " + timeDisplayShort(player.challengeTimes[1])
    document.getElementById("challengetime4").textContent = "普通挑战  " + 4 + " 时间记录: " + timeDisplayShort(player.challengeTimes[6])
    document.getElementById("challengetime5").textContent = "普通挑战  " + 5 + " 时间记录: " + timeDisplayShort(player.challengeTimes[4])
    document.getElementById("challengetime6").textContent = "普通挑战  " + 6 + " 时间记录: " + timeDisplayShort(player.challengeTimes[8])
    document.getElementById("challengetime7").textContent = "普通挑战  " + 7 + " 时间记录: " + timeDisplayShort(player.challengeTimes[7])
    document.getElementById("challengetime8").textContent = "普通挑战  " + 8 + " 时间记录: " + timeDisplayShort(player.challengeTimes[9])
    document.getElementById("challengetime9").textContent = "普通挑战  " + 9 + " 时间记录: " + timeDisplayShort(player.challengeTimes[3])
    document.getElementById("challengetime10").textContent = "普通挑战 " + 10 + " 时间记录: " + timeDisplayShort(player.challengeTimes[2])
    document.getElementById("challengetime11").textContent = "普通挑战 " + 11 + " 时间记录: " + timeDisplayShort(player.challengeTimes[10])
    document.getElementById("challengetime12").textContent = "普通挑战 " + 12 + " 时间记录: " + timeDisplayShort(player.challengeTimes[5])
	var temp = 0
	for (var i=0; i<11; i++) {
		temp += player.challengeTimes[i]
	}
	document.getElementById("challengetimesum").textContent = "普通挑战时间记录之和: " + timeDisplayShort(temp)
	...
	document.getElementById("infchallengetimesum").textContent = "无限挑战时间记录之和: " + timeDisplayShort(temp)
...
function updateLastTenRuns() {
    ...
    for (var i=0; i<10; i++) {
        ...
        var tempstring = shorten(ippm) + " 无限点数每分钟"
        if (ippm<1) tempstring = shorten(ippm*60) + " 无限点数每小时"
        document.getElementById("run"+(i+1)).textContent = "往前第 "+(i+1)+" 次无限用时 " + timeDisplayShort(player.lastTenRuns[i][0]) + " ,获得了 " + shortenDimensions(player.lastTenRuns[i][1]) +" 无限点数. "+ tempstring
    }

    var ippm = tempIP.dividedBy(tempTime/600)
    var tempstring = shorten(ippm) + " 无限点数每分钟"
    if (ippm<1) tempstring = shorten(ippm*60) + " 无限点数每小时"
    document.getElementById("averagerun").textContent = "前 10 次无限平均时间: "+ timeDisplayShort(tempTime)+" 平均无限点数获取: "+shortenDimensions(tempIP)+" 无限点数. "+tempstring
...
function updateLastTenEternities() {
    ...
    for (var i=0; i<10; i++) {
        ...
        var tempstring = shorten(eppm) + " 永恒点数每分钟"
        if (eppm<1) tempstring = shorten(eppm*60) + " 永恒点数每小时"
        document.getElementById("eternityrun"+(i+1)).textContent = "往前第 "+(i+1)+" 次永恒用时 " + timeDisplayShort(player.lastTenEternities[i][0]) + " ,获得了 " + shortenDimensions(player.lastTenEternities[i][1]) +" 永恒点数. "+ tempstring
    }
    ...
    averageEp = tempEP
    if (eppm<1) tempstring = shorten(eppm*60) + " 永恒点数每小时"
    document.getElementById("averageEternityRun").textContent = "前 10 次永恒平均时间: "+ timeDisplayShort(tempTime)+" 平均永恒点数获取: "+shortenDimensions(tempEP)+" 永恒点数. "+tempstring
}
...
document.getElementById("bigcrunch").onclick = function () {
    if(player.thisInfinityTime<1e-7) {console.log('time err: inf in 0s');player.thisInfinityTime=1;}
    ...
    if ((player.money.gte(Number.MAX_VALUE) && !player.currentChallenge.includes("post")) || (player.currentChallenge !== "" && player.money.gte(player.challengeTarget))) {
        ...
        document.getElementById("replicantireset").innerHTML = "重置复制器数量，但是获得一个复制器星系<br>获得了 "+player.replicanti.galaxies + " 个复制器星系."
...
function eternity(force, auto) {
    if ((player.infinityPoints.gte(Number.MAX_VALUE) && (!player.options.eternityconfirm || auto || confirm("永恒将会重置挑战记录和成就以外的内容. 你将会获得永恒点并解锁新升级.") )) || force === true) {
        ...
        player = {
            ...
            timeDimension8: player.timeDimension8,
            eternityPoints: player.eternityPoints,
            eternities: player.eternities+Math.floor(Math.min(1000,Math.max(5,Math.pow(player.eternities,2/3)))),
            thisEternity: 0,
            bestEternity: player.bestEternity,
            eternityUpgrades: player.eternityUpgrades,
            epmult: player.epmult,
        ...
        updateLastTenRuns()
        updateLastTenEternities()
        if (!player.achievements.includes("r133") || player.eternities == 0) {
        ...
        document.getElementById("infinityPoints1").innerHTML = "你有 <span class=\"IPAmount1\">"+shortenDimensions(player.infinityPoints)+"</span> 无限点数."
        document.getElementById("infinityPoints2").innerHTML = "你有 <span class=\"IPAmount2\">"+shortenDimensions(player.infinityPoints)+"</span> 无限点数."
        if (player.eternities < 2) document.getElementById("break").textContent = "打破无限"
        document.getElementById("replicantireset").innerHTML = "重置复制器数量，但是获得一个复制器星系<br>获得了 "+player.replicanti.galaxies + " 个复制器星系."
        ...
        document.getElementById("infiMult").innerHTML = "获得的无限点数乘 2 <br>当前: "+shorten(player.infMult.times(kongIPMult)) +"x<br>花费: "+shortenCosts(player.infMultCost)+" 无限点数"
        updateEternityUpgrades()
        document.getElementById("totaltickgained").textContent = "你获得了 "+player.totalTickGained.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+" 个时间速率升级."
        updateTickSpeed();
        playerInfinityUpgradesOnEternity()
        document.getElementById("eternityPoints2").innerHTML = "你有 <span class=\"EPAmount2\">"+shortenDimensions(player.eternityPoints)+"</span> 永恒点数."
...
function exitChallenge() {
    if (player.currentChallenge !== "") {
        document.getElementById(player.currentChallenge).textContent = "开始"
        startChallenge("");
        updateChallenges();
...
function startChallenge(name, target) {
  if(player.options.challConf || name == "" ? true : (name.includes("post")) ? confirm("你需要重新开始一次无限，并需要在特殊条件下达到指定目标。注意：最右侧一列的无限升级在挑战中无效。") : confirm("你需要重新开始一次无限，并需要在特殊条件下达到无穷。注意：最右侧一列的无限升级在挑战中无效。")) {
    if (player.currentChallenge != "") document.getElementById(player.currentChallenge).textContent = "开始"
    player = {
        money: new Decimal(10),
        tickSpeedCost: new Decimal(1000),
        tickspeed: new Decimal(1000),
...
function startEternityChallenge(name, startgoal, goalIncrease) {
    ...
    if((player.options.challConf) || name == "" ? true :  (confirm("你需要重新开始一次永恒，并需要在特殊条件下达到指定无限点数。"))) {
        player = {
            money: new Decimal(10),
            tickSpeedCost: new Decimal(1000),
            tickspeed: new Decimal(1000),
...
function startDilatedEternity() {
    ...
    if (!confirm("进入时间膨胀将会进入一次新的永恒，且你的全部普通/无限/时间维度和时间速率倍率的指数将会减少到 ^0.75。如果你能在膨胀中永恒，你会根据你的反物质和当前的超光速粒子获得超光速粒子。")) {
        setTimeout(function() {
            gameLoopIntervalId = setInterval(gameLoop, player.options.updateRate);
        }, 250)
        return
...
function unlockDilation() {
    ...
    document.getElementById("dilationunlock").innerHTML = "解锁时间膨胀<span>花费: 5000 时间定理"
}
...
function updateDilationUpgradeButtons() {
    ...
    document.getElementById("dil10desc").textContent = "当前: "+shortenMoney(Math.floor(getTimeTheoremGain()))+" 每秒"
}

function updateDilationUpgradeCosts() {
    document.getElementById("dil1cost").textContent = "花费: " + shortenCosts( new Decimal(DIL_UPG_COSTS[1][0]).times(Decimal.pow(DIL_UPG_COSTS[1][1],(player.dilation.rebuyables[1]))) ) + " 膨胀时间"
    document.getElementById("dil2cost").textContent = "花费: " + shortenCosts( new Decimal(DIL_UPG_COSTS[2][0]).times(Decimal.pow(DIL_UPG_COSTS[2][1],(player.dilation.rebuyables[2]))) ) + " 膨胀时间"
    document.getElementById("dil3cost").textContent = "花费: " + formatValue(player.options.notation, new Decimal(DIL_UPG_COSTS[3][0]).times(Decimal.pow(DIL_UPG_COSTS[3][1],(player.dilation.rebuyables[3]))), 1, 1) + " 膨胀时间"
    document.getElementById("dil4cost").textContent = "花费: " + shortenCosts(DIL_UPG_COSTS[4]) + " 膨胀时间"
    document.getElementById("dil5cost").textContent = "花费: " + shortenCosts(DIL_UPG_COSTS[5]) + " 膨胀时间"
    document.getElementById("dil6cost").textContent = "花费: " + shortenCosts(DIL_UPG_COSTS[6]) + " 膨胀时间"
    document.getElementById("dil7cost").textContent = "花费: " + shortenCosts(DIL_UPG_COSTS[7]) + " 膨胀时间"
    document.getElementById("dil8cost").textContent = "花费: " + shortenCosts(DIL_UPG_COSTS[8]) + " 膨胀时间"
    document.getElementById("dil9cost").textContent = "花费: " + shortenCosts(DIL_UPG_COSTS[9]) + " 膨胀时间"
    document.getElementById("dil10cost").textContent = "花费: " + shortenCosts(DIL_UPG_COSTS[10]) + " 膨胀时间"
}
...
function updateInfPower() {
    ...
    if (player.currentEternityChall == "eterc7") document.getElementById("infPowPerSec").textContent = "你每秒获得 " +shortenDimensions(DimensionProduction(1))+" 第七维度."
    else document.getElementById("infPowPerSec").textContent = "你每秒获得 " +shortenDimensions(DimensionProduction(1))+" 无限之力."
}
...
function updateTimeShards() {
    if (document.getElementById("timedimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
        ...
        if (player.currentEternityChall == "eterc7") document.getElementById("timeShardsPerSec").textContent = "你每秒获得 "+shortenDimensions(getTimeDimensionProduction(1))+" 第八无穷维度."
...
function updateDilation() {
    if (document.getElementById("dilation").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        ...
        document.getElementById("dilatedTimePerSecond").textContent = "+" + shortenMoney(getDilatedTimeGain()) + "/秒"
...
setInterval(function() {
    ...
    document.getElementById("kongip").textContent = "Double your 无限点数 gain from all sources (additive). Forever. 当前: x"+kongIPMult+", next: x"+(kongIPMult==1? 2: kongIPMult+2)
    document.getElementById("kongep").textContent = "Triple your 永恒点数 gain from all sources (additive). Forever. 当前: x"+kongEPMult+", next: x"+(kongEPMult==1? 3: kongEPMult+3)
    document.getElementById("kongdim").textContent = "Double all your normal dimension multipliers (multiplicative). Forever. 当前: x"+kongDimMult+", next: x"+(kongDimMult*2)
    document.getElementById("kongalldim").textContent = "Double ALL the dimension multipliers (Normal, Infinity, Time) (multiplicative until 32x). Forever. 当前: x"+kongAllDimMult+", next: x"+((kongAllDimMult < 32) ? kongAllDimMult * 2 : kongAllDimMult + 32)
    document.getElementById("eternityPoints2").innerHTML = "你有 <span class=\"EPAmount2\">"+shortenDimensions(player.eternityPoints)+"</span> 永恒点数"+"."
    ...
    for (var i=1; i <=8; i++) {
        document.getElementById("postc"+i+"goal").textContent = "目标: "+shortenCosts(goals[i-1])
    }
    ...
    if (player.eternities > 9) document.getElementById("bulklabel").textContent = "每 X 秒最大化维度提升:"
    else document.getElementById("bulklabel").textContent = "批量维度提升数量:"
    ...
    document.getElementById("eterc1completed").textContent = "完成了 "+ECTimesCompleted("eterc1")+" 次."
    ...
    document.getElementById("eterc2completed").textContent = "完成了 "+ECTimesCompleted("eterc2")+" 次."
    ...
    document.getElementById("eterc3completed").textContent = "完成了 "+ECTimesCompleted("eterc3")+" 次."

    document.getElementById("eterc4goal").textContent = "目标: "+Math.max((16 - (ECTimesCompleted("eterc4")*4)), 0)+" 次无限内获得 "+shortenCosts(new Decimal("1e2750").times(new Decimal("1e550").pow(ECTimesCompleted("eterc4"))).max(new Decimal("1e2750"))) + " 无限点数"
    document.getElementById("eterc4completed").textContent = "完成了 "+ECTimesCompleted("eterc4")+" 次."
    ...
    document.getElementById("eterc5completed").textContent = "完成了 "+ECTimesCompleted("eterc5")+" 次."
    ...
    document.getElementById("eterc6completed").textContent = "完成了 "+ECTimesCompleted("eterc6")+" 次."
    ...
    document.getElementById("eterc7completed").textContent = "完成了 "+ECTimesCompleted("eterc7")+" 次."
    ...
    document.getElementById("eterc8completed").textContent = "完成了 "+ECTimesCompleted("eterc8")+" 次."
    ...
    document.getElementById("eterc9completed").textContent = "完成了 "+ECTimesCompleted("eterc9")+" 次."
    ...
    document.getElementById("eterc10completed").textContent = "完成了 "+ECTimesCompleted("eterc10")+" 次."
    ...
    document.getElementById("eterc11completed").textContent = "完成了 "+ECTimesCompleted("eterc11")+" 次."

    document.getElementById("eterc12goal").textContent = "目标: "+(Math.max(10 - ECTimesCompleted("eterc12")*2, 1)/10) + " 秒内获得 " +shortenCosts(new Decimal("1e110000").times(new Decimal("1e12000").pow(ECTimesCompleted("eterc12"))).max(new Decimal("1e110000"))) + " 无限点数"
    document.getElementById("eterc12completed").textContent = "完成了 "+ECTimesCompleted("eterc12")+" 次."
    ...
    if (player.currentEternityChall == "eterc8") {
        ...
        document.getElementById("eterc8repl").textContent = "你还可以购买 "+player.eterc8repl+" 次."
        document.getElementById("eterc8ids").textContent = "你还可以购买 "+player.eterc8ids+" 次."
    } else {
    ...
    document.getElementById("infinitiedBank").textContent = "你存储了 " + player.infinitiedBank.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " 无限次数."
...
function gameLoop(diff) {
    ...
    player.matter = player.matter.times(Decimal.pow((1.03 + player.resets/200 + player.galaxies/100), diff/devSpeed));
    if (player.matter.gt(player.money) && (player.currentChallenge == "challenge12" || player.currentChallenge == "postc1")) {
        if (player.resets > 0) player.resets--;
        softReset(0);
    }

    if (player.currentChallenge == "postc8") postc8Mult = postc8Mult.times(Math.pow(0.000000046416, diff/devSpeed))
    ...
    if (player.timestudy.studies.includes(32)) infGain *= Math.max(player.resets,1);
    infGain *= Math.floor(Math.min(1000,Math.max(5,Math.pow(player.infinitied,2/3))));
    if (player.infinityUpgrades.includes("infinitiedGeneration") && player.currentEternityChall !== "eterc4") player.partInfinitied += diff * infGain / player.bestInfinityTime;
    ...
    player.infinityPoints = player.infinityPoints.plus(gainedInfinityPoints().times(player.offlineProd/100).times(diff/600))
    ...
    if (player.currentEternityChall === "eterc12") player.totalTimePlayed += diff * 1000 / devSpeed
    else player.totalTimePlayed += diff / devSpeed
    player.thisInfinityTime += diff / devSpeed
    player.thisEternity += diff / devSpeed
    ...
    if (IPminpeak.lte("1e100000")) document.getElementById("postInfinityButton").innerHTML = "<b>坍缩以获得 "+shortenDimensions(gainedInfinityPoints())+" 无限点数.</b><br>"+shortenDimensions(currentIPmin) + " 无限点数每分钟"+"<br>最大值为 "+shortenDimensions(IPminpeak)+" 无限点数每分钟"
    else document.getElementById("postInfinityButton").innerHTML = "<b>坍缩以获得 "+shortenDimensions(gainedInfinityPoints())+" 无限点数.</b>"

    if (nextAt[player.postChallUnlocked] === undefined) document.getElementById("nextchall").textContent = " "
    else if (!player.achievements.includes("r133") || player.eternities == 0) {
    ...
    if (player.timestudy.studies.includes(22) ? player.replicanti.interval !== 1 : (player.replicanti.interval !== 50)) document.getElementById("replicantiinterval").innerHTML = "复制间隔: "+(interval).toFixed(3)+" 毫秒<br>-> "+Math.max(interval*0.9).toFixed(3)+" 毫秒 花费: "+shortenCosts(player.replicanti.intervalCost)+" 无限点数"
    else document.getElementById("replicantiinterval").textContent = "复制间隔: "+(interval).toFixed(3)+" 毫秒"
    ...
    document.getElementById("replicantiapprox").textContent ="大概 "+ timeDisplay(estimate*10) + " 后到达无限复制器"
    ...
    document.getElementById("eternitybtn").innerHTML = (player.eternities == 0) ? "还有其他时间.. 我需要永恒." : "<b>我需要永恒.</b><br>"+"获得 <b>"+shortenDimensions(gainedEternityPoints())+"</b> 永恒点数.<br>"+shortenDimensions(currentEPmin)+ " 永恒点数每分钟<br>最大值为 "+shortenDimensions(EPminpeak)+" 永恒点数每分钟"
    if (gainedEternityPoints().gte(1e6)) document.getElementById("eternitybtn").innerHTML = "获得 <b>"+shortenDimensions(gainedEternityPoints())+"</b> 永恒点数.<br>"+shortenDimensions(currentEPmin)+ " 永恒点数每分钟<br>最大值为 "+shortenDimensions(EPminpeak)+" 永恒点数每分钟"
    if (player.dilation.active) document.getElementById("eternitybtn").innerHTML = "获得 <b>"+shortenDimensions(gainedEternityPoints())+"</b> 永恒点数.<br>"+"+"+shortenMoney(Math.round(Math.max(Math.pow(Decimal.log10(player.money) / 400, 1.5) * getTachyonGainMultiplier() - player.dilation.totalTachyonParticles, 0) * 10)/10) +" 超光速粒子."
    if (player.currentEternityChall !== "") document.getElementById("eternitybtn").textContent = "还有其他挑战.. 我需要永恒."
    updateMoney();
    updateCoinPerSec();
    updateDimensions()
    updateInfCosts()
    ...
    document.getElementById("autobuyersbtn").style.display = "none";
    ...
    if (player.money.gte(Number.MAX_VALUE) && (((player.currentChallenge != "" && player.money.gte(player.challengeTarget)) && !player.options.retryChallenge) || (player.bestInfinityTime > 600 && !player.break))) {
        ...
        if (player.infinitied > 0) {
            ...
            document.getElementById("autobuyersbtn").style.display = "inline-block";
        }
    }
    ...
    if (player.currentChallenge !== "") {
        ...
        document.getElementById("progresspercent").setAttribute('ach-tooltip',"到挑战目标的比例")
        ...
        document.getElementById("progresspercent").setAttribute('ach-tooltip',"到无穷的比例")
        ...
        document.getElementById("progresspercent").setAttribute('ach-tooltip',"到下一个维度解锁的比例")
    } else {
        ...
        document.getElementById("progresspercent").setAttribute('ach-tooltip',"到永恒的比例")
    }
    ...
    document.getElementById("ec1reward").textContent = "奖励: 所有时间维度 "+shortenMoney(Math.pow(Math.max(player.thisEternity*10 * devSpeed, 1), 0.3+(ECTimesCompleted("eterc1")*0.05)))+"x (基于本次永恒内的时间)"
    document.getElementById("ec2reward").textContent = "奖励: 无限之力给无限维度 1 一个稍弱的加成, 当前: "+shortenMoney(player.infinityPower.pow(1.5/(700 - ECTimesCompleted("eterc2")*100)).min(new Decimal("1e100")).max(1))+"x"
    document.getElementById("ec3reward").textContent = "奖励: 增加购买 10 个维度的乘数, 当前: "+getDimensionPowerMultiplier().toFixed(2)+"x"
    document.getElementById("ec4reward").textContent = "奖励: 基于未花费的无限点数给无限维度乘数, 当前: "+shortenMoney(player.infinityPoints.pow(0.003 + ECTimesCompleted("eterc4")*0.002).min(new Decimal("1e200")))+"x"
    document.getElementById("ec5reward").textContent = "奖励: 星系花费的加速增长晚 "+((ECTimesCompleted("eterc5")*5))+" 个开始."
    document.getElementById("ec6reward").textContent = "奖励: 继续减少维度价格上涨, 当前: "+player.dimensionMultDecrease.toFixed(1)+"x "
    document.getElementById("ec7reward").textContent = "奖励: 第一时间维度生产第八无穷维度, 当前: "+shortenMoney(getTimeDimensionProduction(1).pow(ECTimesCompleted("eterc7")*0.2).minus(1).max(0))+" 每秒. "
    document.getElementById("ec8reward").textContent = "奖励: 无限之力加强复制器星系, 当前: " + (Math.max(Math.pow(Math.log10(player.infinityPower.plus(1).log10()+1), 0.03 * ECTimesCompleted("eterc8"))-1, 0) * 100).toFixed(2) + "%"
    document.getElementById("ec9reward").textContent = "奖励: 基于时间碎片给无限维度乘数, 当前: "+shortenMoney(player.timeShards.pow(ECTimesCompleted("eterc9")*0.1).min(new Decimal("1e400")))+"x "
    document.getElementById("ec10reward").textContent = "奖励: 基于无限次数给时间维度乘数, 当前: "+shortenMoney(new Decimal(Math.max(Math.pow(getInfinitied(), 0.9) * ECTimesCompleted("eterc10") * 0.000002+1, 1)).pow((player.timestudy.studies.includes(31)) ? 4 : 1))+"x "
    document.getElementById("ec11reward").textContent = "奖励: 继续减少时间速率升级价格上涨, 当前: "+player.tickSpeedMultDecrease.toFixed(2)+"x "
    document.getElementById("ec12reward").textContent = "奖励: 无限维度花费减少. (x^"+(1-ECTimesCompleted("eterc12")*0.008)+")"
    ...
    var scale2 = [" 质子."," 细胞核."," 氢原子."," 病毒."," 红细胞."," 沙粒."," 米粒."," 茶匙."," 酒瓶."," 冰箱."," 游泳池."," 金字塔."," 长城."," 小行星.",
                " 矮行星."," 地球."," 木星."," 太阳."," 红巨星."," 超巨星."," 星云."," 奥尔特云."," 本地泡."," 星系."," 本星系群."," 玉夫座空域."," 可观测宇宙."," 维度.", " 无穷维度.", " 时间维度."];
    var id = 0;
    if (player.money.times(4.22419e-105).gt(2.82e-45)) {
        ...
        if (id >= 7 && id < 11) document.getElementById("infoScale").textContent = "如果每个反物质有 1 普朗克体积大, 你将足够填满 " + formatValue(player.options.notation, player.money * 4.22419e-105 / scale1[id], 2, 1) + scale2[id];
        else document.getElementById("infoScale").textContent = "如果每个反物质有 1 普朗克体积大, 你将足够制造 " + formatValue(player.options.notation, player.money.times(4.22419e-105).dividedBy(scale1[id]), 2, 1) + scale2[id];
    } else { //does this part work correctly? i doubt it does
        if (player.money.times(1e-54) < 2.82e-45) document.getElementById("infoScale").textContent = "如果每个反物质有 " + formatValue(player.options.notation,2.82e-45 / 1e-54 / player.money, 2, 1) + " 立方阿米, 你将足够制造一个质子."
        else if (player.money * 1e-63 < 2.82e-45) document.getElementById("infoScale").textContent = "如果每个反物质有 " + formatValue(player.options.notation,2.82e-45 / 1e-63 / player.money, 2, 1) + " 立方介米, 你将足够制造一个质子."
        else if (player.money * 1e-72 < 2.82e-45) document.getElementById("infoScale").textContent = "如果每个反物质有 " + formatValue(player.options.notation,2.82e-45 / 1e-72 / player.money, 2, 1) + " 立方攸米, 你将足够制造一个质子."
        else document.getElementById("infoScale").textContent = "如果每个反物质有 " + formatValue(player.options.notation,2.82e-45 / 4.22419e-105 / player.money, 2, 1) + " 普朗克体积大, 你将足够制造一个质子."
    }
    if (player.money.gt(new Decimal("1e100000"))) {
        document.getElementById("infoScale").innerHTML = "<br>如果你每秒写 3 个数字, 你将会花 <br>" + timeDisplay(player.money.log10()*10/3) + "<br> 来写下你的反物质的数量.";
    }
    ...
    document.getElementById("newDimensionButton").textContent = "获得 " + shortenCosts(getNewInfReq()) + " 反物质来解锁新的维度."

    document.getElementById("sacrifice").setAttribute('ach-tooltip', "增强第八维度到 " + formatValue(player.options.notation, calcSacrificeBoost(), 2, 2) + "x");
    ...
    if (player.eternityUpgrades.includes(7)) player.eternityPoints = player.eternityPoints.plus(gainedEternityPoints().times(diff/1000))
    if (player.dilation.upgrades.includes(10)) {
        ...
        if (document.getElementById("timestudies").style.display != "none" && document.getElementById("eternitystore").style.display != "none") {
            if (player.timestudy.theorem>99999) document.getElementById("timetheorems").innerHTML = "你有 <span style='display:inline' class=\"TheoremAmount\">"+shortenMoney(player.timestudy.theorem)+"</span> 时间定理."
            else document.getElementById("timetheorems").innerHTML = "你有 <span style='display:inline' class=\"TheoremAmount\">"+player.timestudy.theorem.toFixed(0)+"</span> 时间定理."
            updateTimeStudyButtons()
        }
    }
...
function simulateTime(seconds, real) {
    ...
    var popupString = "你离开的时候"
    if (player.money.gt(playerStart.money)) popupString+= ",<br> 反物质上升了 "+shortenMoney(player.money.log10() - (playerStart.money).log10())+" 个数量级"
    if (player.infinityPower.gt(playerStart.infinityPower)) popupString+= ",<br> 无限之力上升了 "+shortenMoney(player.infinityPower.log10() - (Decimal.max(playerStart.infinityPower, 1)).log10())+" 个数量级"
    if (player.timeShards.gt(playerStart.timeShards)) popupString+= ",<br> 时间碎片上升了 "+shortenMoney(player.timeShards.log10() - (Decimal.max(playerStart.timeShards, 1)).log10())+" 个数量级"
    if (player.infinitied > playerStart.infinitied || player.eternities > playerStart.eternities) popupString+= ","
    else popupString+= "."
    if (player.infinitied > playerStart.infinitied) popupString+= "<br>无限了 "+(player.infinitied-playerStart.infinitied)+" 次."
    if (player.eternities > playerStart.eternities) popupString+= " <br>永恒了 "+(player.eternities-playerStart.eternities)+" 次."
...
function enableChart() {
    if (document.getElementById("chartOnOff").checked) {
        ...
        if (player.options.chart.warning < 1) alert("警告：图表可能会导致性能问题. 如果感到延迟过高, 请禁用它.")
    } else {
...
function autoBuyerTick() {
    ...
    if(player.eternityUpgrades.includes(8)) buyMaxEPMult()
    if(player.eternityUpgrades.includes(9)) {maxTheorems();buyMaxTimeDimensions()}
...
function init() {
    ...
    document.getElementById('autobuyersbtn').onclick=function () {
      showTab('autobuyers');
    };
